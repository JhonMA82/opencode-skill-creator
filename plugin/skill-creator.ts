...
import { createEvidenceDrivenTools } from "./lib/evidence-tools"
...
export const SkillCreatorPlugin: Plugin = async (ctx) => {
  ...
  return {
    tool: {
      ...
      // Fork-specific evidence-driven tools (context lint, behavioral case
      // validation, rationalization collection, regression suite, instruction
      // usefulness) — registered from lib/evidence-tools.ts so this
      // upstream-owned file stays close to upstream.
